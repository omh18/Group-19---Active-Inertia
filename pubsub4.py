...
parser.add_argument('--qos', default=0, type=int, help="Set QoS of messages" +
                                                          "Default is 0")
parser.add_argument('--title', default='Histogram of Delays', help="Set title for histogram")
...
if __name__ == '__main__':
    ...
    print(f"Over 50ms: {len([x for x in DelayList if x >= 0.05])} occurrences")
    print(f"Over 100ms: {len([x for x in DelayList if x >= 0.1])} occurrences")

    DelayListms = [element * 1000 for element in DelayList]

    f = plt.figure()
    ax = f.add_subplot(111)
    plt.style.use('ggplot')
    plt.hist(DelayListms, 100)
    plt.xlabel('Delay in ms')
    plt.ylabel('Frequency')
    plt.title(args.title)
    #plt.text(50, 3, '$\mu={:.2f},\ \sigma={:.2f}$'.format(total*1000/counter, statistics.stdev(DelayListms)))
    plt.text(0.5, 0.9, '$\mu={:.2f},\ \sigma={:.2f}$'.format(total*1000/counter, statistics.stdev(DelayListms)), ha='center', va='center', transform=ax.transAxes)
    plt.grid(True)
    #plt.show()
    plt.savefig('qos0.png')
